...
const DiffBadge = ({ d }: { d: string }) => {
  ...
  return <span style={{ padding: '4px 8px', fontSize: 11, background: `${c}15`, color: c, borderRadius: 4, border: `1px solid ${c}40`, textTransform: 'uppercase', fontWeight: 700 }}>{d}</span>
}

const XPBadge = ({ xp }: { xp: number }) => (
  <span style={{ padding: '4px 8px', fontSize: 11, background: `${T.cyan}15`, color: T.cyan, borderRadius: 4, border: `1px solid ${T.cyan}40`, fontWeight: 700 }}>+{xp} XP</span>
)
...
const Learn = ({ completedModules, onComplete, updateXP }: any) => {
  const [active, setActive] = useState<string | null>(null);
  const [quiz, setQuiz] = useState({ answered:false, selected:null as number|null });
 
  const handleQuiz = (idx: number, mod: any) => {
    if (quiz.answered) return;
    setQuiz({ answered:true, selected:idx });
    if (idx === mod.quiz.ans && !completedModules.includes(mod.id)) { onComplete(mod.id); updateXP(mod.xp); }
  };
  ...
  if (active) {
    ...
    return (
      <div style={{ padding:'80px 24px 60px', maxWidth:680, margin:'0 auto', minHeight: '80vh' }}>
        <button onClick={() => { setActive(null); setQuiz({ answered:false, selected:null }); }} style={{ background:'none', border:'none', color:T.mid, cursor:'pointer', display:'flex', alignItems:'center', gap:6, marginBottom:28, fontFamily:'var(--font-body)', fontSize:14 }}>← Back</button>
        <div className="slide-r">
          <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:10 }}>
            ...
            <div>
              <h2 className="syne" style={{ fontSize:24, fontWeight:800, fontFamily: 'var(--font-head)', letterSpacing: '1px' }}>{mod.title}</h2>
              <div style={{ display:'flex', gap:8, marginTop:6, flexWrap:'wrap' }}>
                <DiffBadge d={mod.difficulty}/><XPBadge xp={mod.xp}/>
                {completedModules.includes(mod.id) && <Tag color={T.green}>✓ Completed</Tag>}
              </div>
            </div>
          </div>
          <div className="glass" style={{ padding:22, marginTop:22, marginBottom:14, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
            <p style={{ color:'#ddd', lineHeight:1.78, fontSize:15 }}>{mod.content}</p>
          </div>
          <div className="glass" style={{ padding:22, marginBottom:22, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
            <div className="mono" style={{ fontSize:11, color:T.dim, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:14, fontFamily: 'var(--font-mono)' }}>⚠ Red Flags to Watch</div>
            {mod.flags.map((f,i) => (
          ...
          <div className="glass" style={{ padding:22, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
            <div className="mono" style={{ fontSize:11, color:T.cyan, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:16, fontFamily: 'var(--font-mono)' }}>⚡ Knowledge Check</div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:9 }}>
              {mod.quiz.opts.map((opt,i) => {
                ...
                return (
                  <button key={i} onClick={() => handleQuiz(i,mod)} style={{ padding:'13px 16px', background:bg, border:`1px solid ${border}`, borderRadius:9, color:col, textAlign:'left', cursor:quiz.answered?'default':'pointer', fontFamily:'var(--font-body)', fontSize:15, display:'flex', alignItems:'center', gap:10, transition:'all 0.18s', fontWeight: 600 }}>
            ...
            {quiz.answered && (
              <div style={{ marginTop:16, padding:16, background:`${T.cyan}08`, border:`1px solid ${T.cyan}33`, borderRadius:8 }}>
                ...
                {quiz.selected===mod.quiz.ans && <div style={{ marginTop:10, color:T.green, fontWeight:600, fontSize:14 }}>+{mod.xp} XP earned!</div>}
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div style={{ padding:'80px 24px 60px', maxWidth:840, margin:'0 auto', minHeight: '80vh' }}>
      <div style={{ marginBottom:32 }}>
        <h2 className="syne" style={{ fontSize:'clamp(32px,6vw,48px)', fontWeight:800, fontFamily: 'var(--font-head)', letterSpacing: '2px', textTransform: 'uppercase' }}>Knowledge <span style={{ color:T.red }}>Database</span></h2>
        <p style={{ color:T.mid, marginTop:6, fontSize:15 }}>{completedModules.length}/{MODULES.length} modules completed</p>
        <div className="pbar" style={{ marginTop:10, maxWidth:240, height: '6px', background: 'rgba(255,255,255,0.05)', borderRadius: '3px', overflow: 'hidden' }}>
          <div className="pfill" style={{ width:`${(completedModules.length/MODULES.length)*100}%`, height: '100%', background:`linear-gradient(90deg,${T.red},${T.purple})`, transition: 'width 0.5s ease-out' }}/>
        </div>
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
        {MODULES.map((mod,i) => (
          <div key={mod.id} className="glass fade-up" style={{ padding:28, cursor:'pointer', animationDelay:`${i*0.07}s`, position:'relative', overflow:'hidden', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px', transition: 'transform 0.2s', boxShadow: '0 4px 24px rgba(0,0,0,0.4)' }} 
               onClick={() => { setActive(mod.id); setQuiz({ answered:false, selected:null }); }}
               onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-2px)'}
               onMouseLeave={(e) => e.currentTarget.style.transform = 'none'}
               >
            <div style={{ position:'absolute', right:-20, top:-20, width:100, height:100, background:`${mod.color}10`, borderRadius:'50%', filter: 'blur(10px)' }}/>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:10, position: 'relative', zIndex: 1 }}>
              ...
              <div style={{ color:T.cyan, display:'flex', alignItems:'center', gap:4, fontSize:14, fontWeight:600 }}>Open <ChevronRight size={18}/></div>
            </div>
            <p style={{ color:'#aaa', fontSize:14, lineHeight:1.65, marginTop:16, position: 'relative', zIndex: 1, maxWidth: '90%' }}>{mod.content}</p>
            <div style={{ marginTop:16, display:'flex', gap:8, flexWrap:'wrap', position: 'relative', zIndex: 1 }}>
              {mod.flags.slice(0,3).map((f,fi) => (
                <span key={fi} style={{ fontSize:12, padding:'4px 10px', background:`${T.red}10`, color: T.red, border:`1px solid ${T.red}33`, borderRadius:6, fontFamily: 'var(--font-body)' }}>⚠ {f.split("'")[0]}</span>
              ))}
            </div>
          </div>
        ))}
...
export default function LearnPage() {
  ...
  const handleUpdateXP = (xp: number) => {
